<!-- Profile Views -->

<p align="right">
  <img src="https://komarev.com/ghpvc/?username=Jathugulan&label=Profile%20Views&color=4F46E5&style=flat" />
</p>

<!-- Header -->

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0F172A,35:312E81,70:4F46E5,100:059669&height=220&section=header&text=Raveendran%20Jathugulan&fontSize=40&fontColor=FFFFFF&fontAlignY=32&desc=Software%20Engineer%20%7C%20Full%20Stack%20Engineer&descSize=17&descAlignY=53&animation=fadeIn" width="100%" />
</p>

<h1 align="center">Hi, I'm Raveendran Jathugulan 👋</h1>

<p align="center">
  <strong>Software Engineer</strong> · Full Stack Engineering · Modern Web Applications · Frontend & Backend Development
</p>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=600&size=18&duration=2800&pause=800&color=4F46E5&center=true&vCenter=true&width=800&lines=Software+Engineer;Full+Stack+Engineer;Building+Modern+Web+Applications;Frontend+%2B+Backend+Engineering;Designing+Scalable+Applications;Turning+Ideas+Into+Software" />
</p>

---

<!-- Social Links -->

<p align="center">

<a href="https://github.com/Jathugulan">
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" />
</a>

<a href="https://www.linkedin.com/in/raveendran-jathugulan/">
  <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
</a>

<a href="https://portfolio-pi-sepia-30.vercel.app/">
  <img src="https://img.shields.io/badge/Portfolio-4F46E5?style=for-the-badge&logo=vercel&logoColor=white" />
</a>

<a href="mailto:jathugulan2022@gmail.com">
  <img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" />
</a>

</p>

---

## 👨‍💻 About Me

I am **Raveendran Jathugulan**, a **Software Engineer and Full Stack Engineer** from Sri Lanka, focused on building modern, scalable and user-focused web applications across frontend, backend and database layers.

* 🎓 **Education:** BICT (Hons.) — University of Vavuniya
* 💻 **Specialization:** Full Stack Software Engineering
* 🚀 **Development:** Modern web applications and enterprise systems
* 🧩 **Frontend:** React, Next.js, TypeScript, JavaScript
* ⚙️ **Backend:** Node.js, Express.js, NestJS, Laravel, PHP
* 🗄️ **Databases:** MongoDB, MySQL, SQL Server
* 🔐 **Security:** JWT, OAuth 2.0, RBAC, bcrypt, secure API practices
* ⚡ **Real-Time:** Socket.IO and event-driven applications
* 📍 **Applications:** Location-aware and map-based systems
* 🤝 **Open to:** Software Engineering, Full Stack opportunities and collaborative projects

> *Building reliable software that turns ideas and real-world problems into practical digital solutions.*

---

## 🧑‍💻 Developer Profile

<table align="center">
<tr>
<td align="center" width="25%">

### 💻

**Software Engineering**

Modern application development

</td>

<td align="center" width="25%">

### 🎨

**Frontend**

Responsive & interactive interfaces

</td>

<td align="center" width="25%">

### ⚙️

**Backend**

Scalable server-side systems

</td>

<td align="center" width="25%">

### 🗄️

**Database**

Structured & efficient data systems

</td>
</tr>
</table>

---

## 🛠️ Technology Stack

### 💻 Programming Languages

<p align="center">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/cplusplus/cplusplus-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/csharp/csharp-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/php/php-original.svg" width="55"/>
</p>

### 🎨 Frontend Engineering

<p align="center">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nextjs/nextjs-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vitejs/vitejs-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/redux/redux-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tailwindcss/tailwindcss-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bootstrap/bootstrap-original.svg" width="55"/>
</p>

### ⚙️ Backend Engineering

<p align="center">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/express/express-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nestjs/nestjs-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/laravel/laravel-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/php/php-original.svg" width="55"/>
</p>

### 🗄️ Databases

<p align="center">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mongodb/mongodb-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/microsoftsqlserver/microsoftsqlserver-plain.svg" width="55"/>
</p>

### 🛠️ Tools & Platforms

<p align="center">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postman/postman-original.svg" width="55"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/figma/figma-original.svg" width="55"/>
</p>

---

## 🚀 Featured Projects

### 🩸 LifeLink

**Emergency Blood Donation & Matching Platform**

A real-time healthcare platform for emergency blood requests, donor matching and location-aware discovery.

**Core Technologies**

`React` `TypeScript` `Node.js` `Express`
`MongoDB` `Socket.IO` `JWT` `Leaflet`

**Advanced Features**

* 🚨 Emergency blood requests
* 🩸 Donor matching
* 📍 Location-based discovery
* ⚡ Real-time notifications
* 🔐 JWT + RBAC
* 📊 Analytics dashboard

<a href="https://github.com/Jathugulan/blood-donation-emergency-matcher">
<img src="https://img.shields.io/badge/View%20Repository-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

---

### 🚗 QuickRide

**Vehicle Rental & Booking Platform**

A complete vehicle rental ecosystem connecting customers, owners, drivers and administrators.

**Core Technologies**

`React` `Vite` `Node.js` `Express`
`MongoDB` `Socket.IO` `Google OAuth` `Cloudinary`

**Advanced Features**

* 🚘 Vehicle management
* 📅 Booking lifecycle
* 👥 Multi-role system
* 💳 Payment integration
* 📍 Maps & route calculation
* ⚡ Real-time communication
* ☁️ Cloudinary integration
* 📧 Email services
* 📖 Swagger/OpenAPI

<a href="https://github.com/Jathugulan/quickride-vehicle-rental-booking">
<img src="https://img.shields.io/badge/View%20Repository-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

---

### 📚 Tholan Book Shop

**AI-Powered E-Commerce Platform**

Modern e-commerce platform with intelligent search, recommendations, chatbot functionality and analytics.

**Core Technologies**

`React` `Redux Toolkit` `Node.js` `Express`
`MongoDB` `JWT` `Leaflet`

**Advanced Features**

* 🛒 E-commerce workflow
* 🤖 Recommendation engine
* 💬 Intelligent chatbot
* 🔎 Natural-language search
* 📊 Analytics
* 📍 Location services
* 🔐 Secure authentication

<a href="https://github.com/Jathugulan/tholan-book-shop">
<img src="https://img.shields.io/badge/View%20Repository-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

---

### 📄 SmartATS

**ATS Resume Builder & Score Analyzer**

A career-focused application for creating ATS-friendly resumes and analyzing PDF/DOCX resumes.

**Core Technologies**

`React 19` `Vite` `Tailwind CSS`
`Node.js` `Express` `MongoDB` `JWT`

**Advanced Features**

* 📄 Resume builder
* 📊 ATS score analysis
* 🔎 Resume parsing
* 📑 PDF/DOCX processing
* 🔐 Secure authentication
* 🛡️ API security
* 🎨 Responsive UI

<a href="https://github.com/Jathugulan/SmartATS-Resume-Builder-Score-Analyzer">
<img src="https://img.shields.io/badge/View%20Repository-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

---

### 👨‍🏫 TeacherPayRoll ERP

**Teacher Attendance, Leave & Payroll Management**

Enterprise-style system designed to manage teacher attendance, leave and payroll workflows.

**Core Technologies**

`React 19` `Vite` `Node.js` `Express`
`MongoDB Atlas` `JWT` `Google OAuth`

**Advanced Features**

* 👨‍🏫 Teacher management
* ⏱️ Attendance
* 🏖️ Leave management
* 💰 Payroll
* 🔐 Authentication
* 🔑 Google OAuth
* 📝 Audit logs
* 📊 Dashboard analytics

<a href="https://github.com/Jathugulan/TeacherPayRollERP">
<img src="https://img.shields.io/badge/View%20Repository-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

---

### 🧠 QuizMaster Pro

**Intelligent Examination & Assessment Platform**

Modern education platform with AI-assisted content generation and performance analytics.

**Core Technologies**

`React 19` `Vite` `Tailwind CSS`
`Node.js` `Express` `MongoDB` `Gemini`

**Advanced Features**

* 📝 Online examinations
* 🤖 AI-generated content
* 📊 Performance analytics
* 👨‍🎓 Assessment management
* 🔐 JWT authentication
* 🛡️ Secure APIs
* ✨ Modern animated UI

<a href="https://github.com/Jathugulan/quizmaster-pro-mern">
<img src="https://img.shields.io/badge/View%20Repository-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

---

## 📦 More Projects

| Project                         | Technology                              | Domain        |
| ------------------------------- | --------------------------------------- | ------------- |
| 🏢 **HRMS Dashboard**           | Next.js · TypeScript · NestJS · MongoDB | HR Management |
| 🍽️ **Vadamārutham Restaurant** | React · Node.js · MongoDB               | Hospitality   |
| 🌐 **Yarl Skill Hub**           | PHP MVC · MySQL · Bootstrap             | Education     |
| 🎓 **Alumni Management**        | PHP · MySQL · Bootstrap                 | University    |
| 💊 **PharmaNova**               | C# · .NET · SQL Server                  | Enterprise    |
| 🍔 **Food Picker Spinner**      | React · Vite · Tailwind                 | Utility       |
| 🧮 **GPA Calculator**           | HTML · CSS · JavaScript                 | Education     |
| 🧮 **SmartCalc Pro**            | HTML · CSS · JavaScript                 | Utility       |
| 🌐 **Portfolio**                | Bootstrap · JavaScript · AOS            | Portfolio     |

---

## 🔐 Security & Authentication

<p align="center">

<img src="https://cdn.simpleicons.org/jsonwebtokens/000000" width="50"/>
<img src="https://cdn.simpleicons.org/oauth/000000" width="50"/>
<img src="https://cdn.simpleicons.org/owasp/000000" width="50"/>

</p>

<p align="center">

<img src="https://img.shields.io/badge/JWT-Authentication-4F46E5?style=for-the-badge"/>
<img src="https://img.shields.io/badge/OAuth-2.0-059669?style=for-the-badge"/>
<img src="https://img.shields.io/badge/RBAC-Authorization-7C3AED?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Helmet-Security-DC2626?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Rate%20Limiting-API%20Protection-EA580C?style=for-the-badge"/>

</p>

---

## ⚡ Advanced Engineering

```text
Frontend
 ├── Component Architecture
 ├── Responsive UI
 ├── State Management
 ├── Form Validation
 └── Interactive Dashboards

Backend
 ├── RESTful Services
 ├── MVC Architecture
 ├── Middleware
 ├── Authentication
 ├── Validation
 └── Error Handling

Database
 ├── Schema Design
 ├── CRUD Operations
 ├── Relationships
 ├── Data Validation
 └── Query Optimization

Application
 ├── Real-Time Communication
 ├── Location Services
 ├── File Processing
 ├── Analytics
 └── Third-Party Integrations
```

---

## 🏆 Certifications

<table align="center">
<tr>
<td align="center">

🚀 **Full Stack Development**

**MasterClass in Full Stack Development**

NoviTech R&D

</td>

<td align="center">

🌐 **MERN Stack**

**Introduction to MERN Stack**

Simplilearn

</td>

<td align="center">

🔥 **MEAN Stack**

**Introduction to MEAN Stack**

Simplilearn

</td>
</tr>

<tr>
<td align="center">

🤖 **Artificial Intelligence**

**Artificial Intelligence**

Pantech.AI

</td>

<td align="center">

🧠 **Machine Learning**

**Machine Learning**

Pantech.AI

</td>

<td align="center">

📊 **Data**

**Data Analytics & Data Science**

Coursera

</td>
</tr>

<tr>
<td align="center">

⚙️ **DevOps**

**Professional Certificate in DevOps**

Udemy

</td>

<td align="center">

☕ **Java**

**Java (Basic)**

HackerRank

</td>

<td align="center">

🎨 **UI/UX**

**UI/UX Design**

Coursera

</td>
</tr>
</table>

---

## 💼 Professional Experience

### Full Stack Web Development Intern

**Pantech.AI** · Jan 2026 – Apr 2026

* Developed full-stack application features
* Integrated frontend and backend systems
* Built REST API functionality
* Worked with database-driven modules

### Full Stack Development Intern

**NoviTech R&D Pvt Ltd** · Apr 2026 – May 2026

* Worked across frontend and backend development
* Followed modern development practices
* Collaborated within team-based workflows

---

## 📊 GitHub Analytics

<p align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=Jathugulan&show_icons=true&count_private=true&hide_border=true&theme=transparent&title_color=4F46E5&icon_color=059669"/>

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Jathugulan&layout=compact&hide_border=true&theme=transparent&title_color=4F46E5"/>

</p>

<p align="center">

<img src="https://github-readme-streak-stats.herokuapp.com/?user=Jathugulan&hide_border=true&theme=transparent&ring=4F46E5&fire=059669&currStreakLabel=4F46E5" width="70%"/>

</p>

---

## 📈 Contribution Activity

<p align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=Jathugulan&bg_color=ffffff&color=334155&line=4F46E5&point=059669&area=true&hide_border=true" width="100%"/>

</p>

---

## 🏅 GitHub Trophies

<p align="center">

<img src="https://github-profile-trophy.vercel.app/?username=Jathugulan&theme=flat&no-frame=true&no-bg=true&margin-w=8&row=2&column=6" width="100%"/>

</p>

---

## 🐍 Contribution Snake

<p align="center">

<img src="https://raw.githubusercontent.com/Jathugulan/Jathugulan/output/github-contribution-grid-snake.svg" width="100%" alt="GitHub Contribution Snake"/>

</p>

---

## 🎯 Engineering Roadmap

| ✅ Experienced          | 🔄 Improving | 🚀 Next                   |
| ---------------------- | ------------ | ------------------------- |
| Full Stack Development | TypeScript   | Advanced System Design    |
| React                  | Testing      | Cloud Architecture        |
| Node.js                | Performance  | Advanced DevOps           |
| MongoDB                | CI/CD        | Scalable Systems          |
| REST APIs              | Docker       | Distributed Systems       |
| Authentication         | Architecture | Cloud-Native Applications |

---

## 🤝 Open to Opportunities

I'm interested in:

**Software Engineering** · **Full Stack Engineering** · **Web Application Development** · **Collaborative Projects** · **Open Source**

---

## 📬 Let's Connect

<p align="center">

<a href="https://github.com/Jathugulan">
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
</a>

<a href="https://www.linkedin.com/in/raveendran-jathugulan/">
<img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>

<a href="https://portfolio-pi-sepia-30.vercel.app/">
<img src="https://img.shields.io/badge/Portfolio-4F46E5?style=for-the-badge&logo=vercel&logoColor=white"/>
</a>

<a href="mailto:jathugulan2022@gmail.com">
<img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

</p>

<p align="center">

<b>Building Software • Solving Problems • Learning Continuously • Creating Impact</b>

</p>

<p align="center">

<i>"Turning ideas into reliable, scalable and meaningful software."</i>

</p>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:059669,50:4F46E5,100:0F172A&height=120&section=footer" width="100%"/>
